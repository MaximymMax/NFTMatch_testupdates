...
window.updateTelegramBackButton = function(mode) {
    ...
    // Снимаем старые клики
    tg.BackButton.offClick();

    if (mode === 'details') {
        // Если открыты детали - кнопка закрывает детали
        tg.BackButton.show();
        tg.BackButton.onClick(() => {
            // Проверяем, есть ли функция закрытия в глобальной области или внутри модуля
            if (window.closeDetailsModal) {
                window.closeDetailsModal();
            } else {
                // Фолбек, если функция внутри background-finder.js не глобальна
                // Пытаемся кликнуть на крестик
                const closeBtn = document.getElementById('modal-close-btn');
                if (closeBtn) closeBtn.click();
            }
        });
    } else {
        // Если мы в списке (findBgs/findModels)
        tg.BackButton.show();
        tg.BackButton.onClick(() => {
