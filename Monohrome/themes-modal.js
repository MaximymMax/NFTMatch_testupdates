...
async function renderModelDetailView(modelData, preloadedData = null) {
    ...
    if (preloadedData && preloadedData.bgData) {
        ...
        if (bg.name) {
            ...
            const bgLinkUrl = `../Monohrome/background-finder.html?mode=findModels&gift=${encodeURIComponent(modelData.GiftName)}&color=${encodeURIComponent(bg.name)}`;
    ...
    modalContent.innerHTML = `
        <div style="padding-bottom: 2rem; position: relative;"> 
            ...
            <div class="tm-info-table">
                <div class="tm-info-row"><span class="tm-info-label">Модель</span><a href="../Monohrome/background-finder.html?mode=findBgs&gift=${encodeURIComponent(modelData.GiftName)}&model=${encodeURIComponent(modelData.ModelName)}" class="tm-info-value link-style" title="Найти фоны для этой модели">
