    ...
    function initTelegramData() {
        if (window.Telegram && window.Telegram.WebApp) {
            ...
            tg.BackButton.onClick(() => {
                // Две точки в начале поднимают на уровень выше (из папки Monohrome в корень проекта)
...
function updateUrlState(modalData = null) {
    ...
    async function toggleNFTsSection() {
        const header = document.getElementById('nfts-toggle-header');
        const grid = document.getElementById('nfts-grid-container');
        const arrow = header.querySelector('.nfts-arrow');
        
        // --- БЛОКИРОВКА РАСКРЫТИЯ ПРИ ОТСУТСТВИИ ПОДПИСКИ ---
        
        // Если сейчас закрыто (мы пытаемся открыть)
        if (!nftsState.isExpanded) {
            // Если данных нет, пробуем загрузить
            if (grid.children.length === 0) {
                try {
                    // Передаем true, чтобы ошибка "вылетела" наверх
                    await loadMoreNFTs(true); 
                } catch (e) {
                    // ЕСЛИ ОШИБКА (нет подписки):
                    console.warn("Раскрытие списка прервано: требуется подписка");
                    // Снимаем фокус с элемента (чтобы не подсвечивался, если это кнопка)
                    if (document.activeElement) document.activeElement.blur();
                    // Прерываем функцию. Код ниже НЕ выполнится.
                    return; 
                }
            }
        }
        
        // --- ВИЗУАЛЬНОЕ ПЕРЕКЛЮЧЕНИЕ (Только если ошибок не было) ---
        
        nftsState.isExpanded = !nftsState.isExpanded;
        
        if (nftsState.isExpanded) {
            header.style.color = '#fff';
            header.classList.add('expanded'); // Если есть CSS класс для жирности/цвета
   ...
   // 3. Загрузка данных
    async function loadMoreNFTs(throwError = false) {
        ...
        const url = `${SERVER_BASE_URL}/api/ListGifts/SearchGifts/${nftsState.page}/${nftsState.pageSize}`;
        ...
        try {
            const data = await secureFetch(url, body);
            ...
            if (data && data.Items && data.Items.length > 0) {
                ...
                if (data.Items.length < nftsState.pageSize) {
                    ...
                    nftsState.page++;
                    setupNFTIntersectionObserver(); // Следим за скроллом
                }
            } else {
                nftsState.hasMore = false;
            }
            ...
            // ВАЖНО: Если мы просили выкинуть ошибку (при первом открытии), делаем это
    ...
    async function applyUrlParameters() {
        ...
        const dGift = urlParams.get('d_gift');
        const dModel = urlParams.get('d_model');
        const dBg = urlParams.get('d_bg');

        if (paramMode) {
            // 1. Переключаем режим, НО ЗАПРЕЩАЕМ обновлять URL (false)
            switchMode(paramMode, false);

            if (paramMode === 'findModels' && paramGift && paramColor) {
                // Ищем гифт без учета регистра
                const foundGift = state.giftNames.find(g => g.toLowerCase() === paramGift.toLowerCase());
                
                if (foundGift) {
                    state.findModels.selectedGift = foundGift;
                    dropdowns.giftModels.value.textContent = foundGift;
                    await fetchAllModelNames(foundGift, false);
                }
                const colorData = fixedColors.find(c => c.id === paramColor || c.name === paramColor);
                if (colorData) {
            ...
            else if (paramMode === 'findBgs' && paramGift) {
                // Ищем гифт без учета регистра
                const foundGift = state.giftNames.find(g => g.toLowerCase() === paramGift.toLowerCase());
                
                if (foundGift) {
                    state.findBgs.selectedGift = foundGift;
                    dropdowns.giftBgs.value.textContent = foundGift;
                    await fetchAllModelNames(foundGift, true); // Ждем загрузки списка моделей
                }
                
                if (paramModel) {
                    // Ищем модель без учета регистра
                    let modelData = state.modelNames.find(m => m.NameModel === paramModel);
                    if (!modelData) {
                        modelData = state.modelNames.find(m => m.NameModel.toLowerCase() === paramModel.toLowerCase());
                    }
                    
                    if (modelData) {
                        const correctModelName = modelData.NameModel;
                        state.findBgs.selectedModel = correctModelName;
                        dropdowns.modelBgs.value.textContent = correctModelName;
                        
                        displayMonocolorAlert(correctModelName);
                        updateModelThemes(correctModelName);
                        setupInPageColorPicker(); // Запускаем поиск
                    }
                }
            }
            
            // 2. Теперь, когда все загружено, обновляем URL (чтобы он стал "чистым")
            updateUrlState();
        }

        // Восстановление модалки
        if (viewMode === 'details' && dGift && dModel && dBg) {
             try {
                const colorObj = fixedColors.find(c => c.name === dBg || c.id === dBg);
                const bgGradient = colorObj ? colorObj.gradient : 'var(--surface-color)';
                
                const realGift = state.giftNames.find(g => g.toLowerCase() === dGift.toLowerCase()) || dGift;
                
                // --- НОВЫЙ КОД: Загружаем данные перед открытием ---
                let fetchedCompat = '...';
                let fetchedCount = null;

                try {
                    // Параллельный запрос данных для скорости
                    const [compatData, countData] = await Promise.all([
                        // 1. Совместимость
                        secureFetch(`${SERVER_BASE_URL}/api/MonoCoof/TopBackgroundColorsByNFT`, {
                            NameGift: realGift,
                            NameModel: dModel
                        }).catch(() => []), // Игнорируем ошибки, вернем пустой массив

                        // 2. Количество
                        secureFetch(`${SERVER_BASE_URL}/api/BaseInfo/GetCountsForModelsAndBackgrounds`, [{
                            NameGift: realGift,
                            NameModel: dModel,
                            BackgroundName: dBg
                        }]).catch(() => [])
                    ...
                    if (Array.isArray(compatData)) {
                        const match = compatData.find(x => x.Key === dBg);
                        if (match) {
                            fetchedCompat = (match.Value * 100).toFixed(1);
                        }
                    }

                    // Обработка количества
                    if (Array.isArray(countData) && countData.length > 0) {
                        fetchedCount = countData[0].Count;
                    }

                } catch (fetchErr) {
                    console.error("Ошибка при дозагрузке данных модалки:", fetchErr);
                }
                // ---------------------------------------------------
                ...
                openDetailsModal({
                    ...
                    compatValue: fetchedCompat, // Теперь передаем реальные данные
                    count: fetchedCount         // Теперь передаем реальные данные
                });
            } catch (e) {
                console.error("Ошибка восстановления модалки:", e);
            }
        }
    }
    ...
    async function init() {
        ...
        // Теперь применяем параметры из ссылки. 
        // URL не сбросится, потому что switchMode выше был с false,
        // а applyUrlParameters вызовет updateUrlState() только в самом конце.
