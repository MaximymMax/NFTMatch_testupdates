document.addEventListener('DOMContentLoaded', () => {
    ...
    function getApiAuthHeader() {
        // 1. Пытаемся получить initData ИЗ КЭША (если был редирект)
        try {
        ...
        if (window.Telegram && window.Telegram.WebApp && window.Telegram.WebApp.initData) {
            ...
            if (directInitData) {
                console.warn('[AUTH] Using direct initData (fallback) and saving to sessionStorage.');
                // Сохраняем его, чтобы он был доступен
    ...
    async function renderSimilarButton(container, giftName, modelName) {
        ...
        try {
            ...
            // --- РЕНДЕРИНГ ---
            const href = `/nft-page/index.html?giftName=${encodeURIComponent(giftName)}&modelName=${encodeURIComponent(modelName)}&randomGiftsCount=10`;
